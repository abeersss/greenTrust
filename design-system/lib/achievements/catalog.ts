 ...
 * Night Shift), 07 (Data Guardian), 09 (Agent Zero), and 10 (GRC
 * Strategist) have real medal graphics and a live challenge behind
 * them -- their labs are built and shipping XP and badges today, so
 * leaving them without medal art meant a player who genuinely earned
 * the badge (row in `user_badges`, correct score) saw an unstyled
 * placeholder pill on /account instead of a medal, which reads as
 * "the badge didn't get awarded" even though it did.
...
export interface AchievementCatalogEntry {
   ...
   * Only `true` for 01, 02, 03, 07, 09, and 10 until each subsequent medal is designed. */
  hasMedalArt: boolean;
}
...
export const ACHIEVEMENT_CATALOG: AchievementCatalogEntry[] = [
  ...
  { number: "09", key: "agentZero", challengeKey: "agent-zero", academy: "aiTrust", hasMedalArt: true },
