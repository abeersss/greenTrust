...
import { safeAuthErrorMessage, buildEmailRedirectTo, rateLimitRetryMessage } from "./auth-helpers";
...
export async function registerUser(input: RegisterInput): Promise<ActionResult> {
  ...
  const ip = await getClientIp();
    ...
    if (!rateLimit.success) {
          return actionError("Too many attempts. Please try again in a minute.", undefined, 60);
    }
  ...
  if (signUpError || !signUpData.user) {
      ...
      // Supabase's own confirmation-email resend cooldown
      // (over_email_send_rate_limit) is separate from our app-level
      // Upstash rate limit above, and fires when the same address
      // requests a confirmation email again too soon. In practice this
      // is almost always a visitor retrying after their first attempt
      // already succeeded and sent the email, so the raw Supabase text
      // ("For security purposes, you can only request this after N
      // seconds") is actively misleading here -- it reads like a
      // failure, not "we already sent it, check your inbox." Production
      // bug found 2026-08-21: a user who clicked Create account
      // repeatedly saw this exact unchanged-looking message every time
      // with no indication anything was different or any path forward.
      // rateLimitRetryMessage (auth-helpers.ts) returns locale-aware
      // guidance instead, and we pass the wait time back so the form
      // can disable the button and count down rather than leaving it
