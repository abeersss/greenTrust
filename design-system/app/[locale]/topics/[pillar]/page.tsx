...
export default async function TopicPage({
  ...
  throw new Error('DEBUG pillar codes=' + JSON.stringify([...pillar].map(c=>c.codePointAt(0))) + ' locale=' + locale + ' pillarLen=' + pillar.length);
    ...
    const __msg = __diagErr instanceof Error ? (__diagErr.stack || __diagErr.message) : String(__diagErr);
    return (
      <pre style={{ whiteSpace: "pre-wrap", padding: 24, fontSize: 12, color: "red", background: "white" }}>
        DIAGNOSTIC ERROR CAPTURE (temporary):
        {"\n"}
        {__msg}
      </pre>
    );
