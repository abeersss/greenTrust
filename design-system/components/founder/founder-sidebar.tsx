...
/**
 * Founder Dashboard nav shell (CyberAbeer Platform Phase II, Phase 1:
 * "Founder auth gate + DB schema + bare dashboard shell"). Only
 * "Dashboard" is a working route in this phase -- every other entry
 * is the founder's own planned Phase II section list, shown so the
 * eventual structure is visible without pretending it already works.
 * Each becomes a real Link href as its own phase ships; until then it
...
const NAV_ITEMS: { key: string; label: string; href?: string }[] = [
  ...
  { key: "intelligence", label: "Cyber Intelligence" },
  { key: "labs", label: "Labs" },
