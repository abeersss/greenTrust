...
const COPY = {
  ...
  passedHeading: { en: "CONTAINMENT COMPLETE — PASSED", ar: "اكتمل الاحتواء — نجاح" },
  notPassedHeading: { en: "Below passing score", ar: "أقل من درجة النجاح" },
  notPassedBody: {
    en: `You scored below the ${BADGE_PASS_SCORE}% threshold required for the badge. Contain another agent with sharper calls to raise your score.`,
    ar: `حصلت على نتيجة أقل من الحد المطلوب البالغ ${BADGE_PASS_SCORE}% للحصول على الشارة. احتوِ وكيلًا آخر بقرارات أدق لرفع نتيجتك.`,
  },
  badgeLockedNote: {
...
export function AgentZeroChallenge({
  ...
  function handleClaimed(result: { xpAwarded: number; badgeAwarded: boolean }) {
    setClaimed(true);
    const localResult = computeAgentZeroScore({ decisions, investigatedClueIds });
